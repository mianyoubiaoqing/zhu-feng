...
34 张棋盘图片均绑定在 `BoardView` 的 Inspector 属性中；金币绑定在 `HUD/Sidebar/MoneyIcon`。标题原图绑定在 `StartMenu/MenuPanel/Title`，并通过 `AtlasTexture` 只显示非透明有效区域。导风板使用四向图片，单向风阀由横竖阀体和方向箭头组合，供能涡轮由底座与旋转叶片组合。当前没有风种、挡风板与门的对应图片，因此这三类对象继续使用程序绘制，避免把语义不符的图片强行复用。
