...
func _draw() -> void:
	...
	if wind != null:
		for cell in wind.cells():
			if wind.is_conflict(cell):
				_draw_cell_texture(conflict_texture, cell, 0.72)
	...
	for door in level.doors:
		...
		_draw_token(door.cell, "门", Color("bfe3ca") if open else Color("df9e9e"))
...
func _draw_terrain_cell(level: LevelDefinition, cell: Vector2i) -> void:
	...
	draw_rect(rect, Color("b9c9c8"), false, 1.0)
...
func _draw_wind_track(cell: Vector2i, direction: int) -> void:
	...
	if track_arrow_texture != null:
		...
		_draw_rotated_cell_texture(track_arrow_texture, cell, arrow_rotation, 0.56)
...
func _draw_device(device: PlacedDevice) -> void:
	...
	match device.kind:
		GameRules.DeviceKind.BLOCKER:
			draw_rect(Rect2(center - Vector2(31, 12), Vector2(62, 24)), Color("7a5a44"), true)
...
func _draw_placement_preview() -> void:
	...
	var alpha := 0.28 if debug_placement_legal else 0.58
	_draw_cell_texture(texture, debug_preview_cell, 0.92, Color(1, 1, 1, alpha))
...
func _draw_failure_feedback() -> void:
	...
	_draw_cell_texture(texture, _result.failure_cell, 0.84)
	if texture == null:
...
func _draw_rotated_cell_texture(texture: Texture2D, cell: Vector2i, rotation: float, scale := 1.0, modulate := Color.WHITE) -> void:
	if texture == null:
		return
	var size := Vector2.ONE * cell_size * scale
	draw_set_transform(cell_center(cell), rotation)
