...
static func build_fan_rotation() -> LevelDefinition:
	var level := _base(&"fan_rotation", "转动风机", "让唯一的风机吹向风种。", "风机提供6级风力，每前进一格减少1级。本关另有2金币，可试放三种装置后退款。")
	level.size = Vector2i(7, 5)
	level.budget = 2
	level.efficient_budget = 0
	level.start = Vector2i(1, 2)
	level.goal = Vector2i(6, 2)
	level.fans = [FanDefinition.create(Vector2i(0, 2), GameRules.Direction.UP, 6)]
	return level


static func build_first_bend() -> LevelDefinition:
	var level := _base(&"first_bend", "第一道弯", "用一块导风板把横向风转向上方终点。", "转弯会额外损失1级风力；数字徽标显示每格剩余风力。")
	level.size = Vector2i(7, 6)
	level.budget = 4
	level.efficient_budget = 2
	level.start = Vector2i(1, 4)
	level.goal = Vector2i(5, 1)
	level.fans = [FanDefinition.create(Vector2i(0, 4), GameRules.Direction.RIGHT, 9)]
	level.walls = [Vector2i(6, 4)]
	return level


static func build_pit_detour() -> LevelDefinition:
	var level := _base(&"pit_detour", "绕开深渊", "用两次转弯绕过下方裂隙。", "13级风足以走完两次转弯；较短的绕路会保留更多速度。")
	level.size = Vector2i(9, 6)
	level.budget = 6
	level.efficient_budget = 4
	level.start = Vector2i(1, 4)
	level.goal = Vector2i(8, 1)
	level.fans = [FanDefinition.create(Vector2i(0, 4), GameRules.Direction.RIGHT, 13)]
	level.pits = [Vector2i(3, 4), Vector2i(4, 4), Vector2i(5, 4)]
	return level


static func build_valve_isolation() -> LevelDefinition:
	var level := _base(&"valve_isolation", "风阀分流", "保留横向运输风，同时让下方风机为涡轮供能。", "交叉风会形成乱流；单向风阀只让朝向一致的风通过。涡轮至少需要2级风。")
	level.size = Vector2i(9, 6)
	level.budget = 4
	level.efficient_budget = 2
	level.start = Vector2i(1, 2)
	...
	level.fans = [
		FanDefinition.create(Vector2i(0, 2), GameRules.Direction.RIGHT, 8),
		FanDefinition.create(Vector2i(4, 5), GameRules.Direction.UP, 6),
	]
...
static func build_dual_supply() -> LevelDefinition:
	var level := _base(&"dual_supply", "双路供能", "一条风路运送风种，另一条风路开启终点前的门。", "涡轮吸收1级风力。上方风机负责供能，下方11级风负责长距离运输。")
	level.size = Vector2i(9, 6)
	level.budget = 4
	level.efficient_budget = 2
	level.start = Vector2i(1, 4)
	...
	level.fans = [
		FanDefinition.create(Vector2i(0, 4), GameRules.Direction.RIGHT, 11),
		FanDefinition.create(Vector2i(0, 0), GameRules.Direction.UP, 6),
	]
...
static func build_full_workshop() -> LevelDefinition:
	var level := _base(&"full_workshop", "风路工坊", "让风种抵达终点；供能开门或绕开门都算完成。", "主风机为15级。可供能开门，也可借逆风抵消、导风绕行，从终点上方或下方进入。")
	...
	level.fans = [
		FanDefinition.create(Vector2i(0, 2), GameRules.Direction.RIGHT, 15),
		FanDefinition.create(Vector2i(3, 5), GameRules.Direction.UP, 6),
		FanDefinition.create(Vector2i(10, 4), GameRules.Direction.LEFT, 6),
	]
